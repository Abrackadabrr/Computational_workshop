...
decltype(auto) integrate_over_cell(const Parametrization &r, const Callable &function) {
...
decltype(auto) integrate_function_over_cell(const Callable &function, const INMOST::Element &cell) {
    ...
    return detail::integrate_over_cell<Quadrature>(r, function) *
        detail::measure<Types::scalar>(cell);
}
 ...
 */
template <typename Quadrature, typename Callable>
decltype(auto) integrate(const Callable &function, INMOST::Mesh &mesh, const Mesh::CellType &cellType) {
